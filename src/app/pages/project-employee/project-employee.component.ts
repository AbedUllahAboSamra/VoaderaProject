...
import { Employee, Task } from '../../model/class/Employee';
...
export class ProjectEmployeeComponent implements OnInit {
  ...
  //   if (id == null) return '—';
  //   return this.employees().find((emp) => emp.id === id)?.fullName;
  // }
  ...
  constructor() {
    ...
    this.masterService.getAllTasks().subscribe((res) => {
      console.log(res);

      this.tasksSignal.set(res ?? []);
    });

    this.masterService.getAllEmp().subscribe((res) => {
  ...
  taskEmployeeForm: FormGroup = this.fb.group({
    id: [null],
    taskId: ['', Validators.required],
    empId: ['', Validators.required],
    assignedDate: [this.todayString(), Validators.required],
    role: ['', Validators.required],
    allocationPct: [0, [Validators.min(0), Validators.max(200)]],
    isActive: [true],
    notes: [''],
  });
  ...
  onEdit(item: any) {
    ...
    this.taskEmployeeForm.patchValue({
      id: item.id,
      taskId: item.taskId,
      empId: item.empId,
  ...
  onSave() {
    const value = this.taskEmployeeForm.value;

    // هنا لاحقاً تربطه بـ API أو Local store
    console.log('SAVE TASK ASSIGNMENT:', value);

    this.toast.success({
      title: 'Saved',
      description: 'Task assignment saved successfully',
    });

    this.taskEmployeeForm.reset({
      assignedDate: this.todayString(),
      isActive: true,
      allocationPct: 0,
    });
  ...
  onDelete(id: number) {
    console.log('delete task assignment', id);
  }
  ...
  taskTitleFor(id: number | null | undefined) {
    if (id == null) return '—';
    return this.tasks().find((t) => t.id === id)?.title;
  }
